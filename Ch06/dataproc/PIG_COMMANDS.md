...
grunt> grouped =
GROUP FLIGHTS2 BY (distbin, depdelaybin);

# Calc ontime
grunt> result =
FOREACH grouped GENERATE FLATTEN(group) AS (dist, delay), ((double)SUM(FLIGHTS2.ontime))/COUNT(FLIGHTS2.ontime) AS ontime:double;

# Upload to gs
grunt> STORE result into 'gs://elite-caster-125113/flights/pigoutput/' using PigStorage(',', '-schema');
```
